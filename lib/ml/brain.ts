...
import { addTriple } from './knowledge-graph';
  ...
  for (const connAB of directConnections) {
    for (const connBC of directConnections) {
      ...
      if (connAB.tagB === connBC.tagA && connAB.tagA !== connBC.tagB) {
        ...
        const cohesion = BrazilianCultureArchitect.calculateCohesion(connAB.tagA, connBC.tagB);
        const gatedStrength = propagatedStrength * (0.4 + cohesion * 0.6);
        
        if (gatedStrength > 0.3) {
          propagated.push({
            tagA: connAB.tagA,
            tagB: connBC.tagB,
            ...
            evidence: [
              `Cadeia cultural: "${connAB.tagA}" → "${connAB.tagB}" → "${connBC.tagB}"`,
              `Coesão cultural: ${Math.round(cohesion * 100)}%`,
          ...
          insights.push(
            `Conexão propagada: "${connAB.tagA}" se liga a "${connBC.tagB}" via cadeia "${connAB.tagB}" (coesão: ${Math.round(cohesion * 100)}%, confiança: ${Math.round(gatedStrength * 100)}%)`
          );
        }
      }
    }
...
export async function runBrainAnalysis(
  ...
  for (const conn of connections.slice(0, 20)) {
    ...
    try {
      await addTriple(conn.tagA, 'related_to', conn.tagB, {
        confidence: conn.strength,
        layer: conn.connectionType === 'propagated' ? 'inferred' : 'factual',
        source: 'inferred',
        mechanism: `brain_${conn.connectionType}`,
      });
    } catch { /* silent */ }
  }
