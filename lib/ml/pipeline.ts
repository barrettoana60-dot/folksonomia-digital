...
export async function runSemanticPipeline(tag: string, obraId: string, visitanteId?: string) {
  ...
  // ============================================================
  // Propagação cerebral + cadeia cultural (aprendizado contínuo)
  // ============================================================
  let brainState = null;
  try {
    const { data: allTagsRaw } = await supabase.from('tags').select('tag_original').limit(300);
    const allTags = (allTagsRaw || []).map((t: any) => t.tag_original);
    brainState = await runBrainAnalysis(tag, allTags);
  } catch {
    // Pipeline continua mesmo se brain falhar
  }
  ...
  return {
    ...
    // Propagação cerebral e cadeia cultural
    brain: brainState ? {
      connections: brainState.totalConnections,
      traces: brainState.totalTraces,
      propagatedInsights: brainState.propagatedInsights.slice(0, 5),
      dnaSignature: brainState.dnaSignature,
    } : null,
