...
type VaultNode = { id: string; label: string; eixo?: string; familia?: string; cor?: string; descricao?: string; x: number; y: number };
...
export default function CulturalInteroperabilityView({ initialNodes = [] }: CulturalInteroperabilityViewProps) {
  const [nodes, setNodes] = useState<VaultNode[]>(defaultNodes);
  const [connections, setConnections] = useState<VaultConnection[]>(defaultConnections);
  ...
  const applyNodes = useCallback((items: any[]) => {
    ...
    setNodes(old => {
      ...
      return valid.map((item, index) => {
        ...
        return { id: item.id?.toString() || keyOf(item.label), label: item.label, eixo: item.eixo, familia: item.familia, cor: item.cor || COLORS[item.eixo] || COLORS.default, descricao: item.description, x: existing?.x ?? 400 + Math.cos(angle) * radius, y: existing?.y ?? 215 + Math.sin(angle) * radius };
      });
    });
  }, []);
  ...
  useEffect(() => {
    ...
    fetch('/api/interop/live-vault').then(response => response.ok ? response.json() : null).then(payload => {
      if (payload?.success && Array.isArray(payload.data?.nodes)) applyNodes(payload.data.nodes);
