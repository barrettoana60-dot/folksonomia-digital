 ...
 */
async function fetchAllDatabaseTags(): Promise<{ id: string; label: string; eixo: string; familia: string; isFromDB: boolean; usageCount: number; sources: string[] }[]> {
  const result: { id: string; label: string; eixo: string; familia: string; isFromDB: boolean; usageCount: number; sources: string[] }[] = [];
  ...
  try {
    ...
    (tagsDB || []).forEach(t => {
      ...
      } else if (isValidCulturalTag(label)) {
        const existing = result.find(item => normalizeForComparison(item.label) === norm);
        if (existing) {
          existing.usageCount += 1;
          if (!existing.sources.includes('tags')) existing.sources.push('tags');
          existing.isFromDB = true;
        }
      }
    });
  } catch (err) {
  ...
  for (let left = 0; left < tags.length; left += 1) {
    for (let right = left + 1; right < tags.length; right += 1) {
      ...
      connections.push({
        ...
        fromLabel: a.label,
        toLabel: b.label,
        weight: Math.round((0.55 + similarity * 0.25 + cohesion * 0.2) * 100) / 100,
        discovered: !validation.valid,
        mechanism: validation.valid ? 'has' : 'inferred',
        afirmacao: `"${a.label}" interliga-se a "${b.label}" por coesão cultural e atributos semânticos compartilhados.`
      });
    }
  }
  return connections
    .sort((a, b) => b.weight - a.weight)
    .slice(0, Math.max(tags.length * 12, 100));
}
...
async function buildDynamicTagDossier(tagLabel: string, allTags: string[] = []): Promise<any> {
  ...
  const [brasilianaResults, europeanaResults, ibramResults] = await Promise.all([
  ...
  const externalEvidence = [...brasilianaResults, ...europeanaResults, ...ibramResults]
    .filter((item, index, all) => all.findIndex(other => other.external_id === item.external_id) === index)
    .slice(0, 15);
...
export async function GET(req: NextRequest) {
  try {
    ...
    const connections = buildTagConnections(nodes.map(node => ({ id: node.id, label: node.label })));
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        totalUserTagUses: allTags.reduce((sum, tag) => sum + tag.usageCount, 0)
      }
    });
  } catch (error: any) {
