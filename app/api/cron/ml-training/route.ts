...
import { processTrainingBatch, getLearningMetrics } from '@/lib/ml/training-loop';
...
async function searchBrasilianaTeoria(query: string): Promise<any[]> {
  return searchAcademicLiterature(query, { maxResults: 8 });
}
...
export async function GET(request: Request) {
  try {
    ...
    // 0. Ciclo de aprendizado progressivo (rede cadeada + MLP + RAG acadêmico)
    const progressiveResults = await processTrainingBatch(3);
    const progressiveMetrics = await getLearningMetrics();
    if (progressiveResults.length > 0) {
      console.log(`[CRON] Aprendizado progressivo: ${progressiveResults.length} tag(s) processada(s)`);
    }
