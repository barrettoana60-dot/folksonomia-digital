...
import { runSpreadingActivation } from '@/lib/ml/graph-math';
import { generateSignature } from '@/lib/core/crypto';
  ...
  try {
    ...
    const baseNodes = [
      {
        ...
        hash: generateSignature({ domain: 'folksonomia-digital/semantic-core/v1', id: 'core' }),
    ...
    for (const tag of tags) {
      ...
      const hash = generateSignature({
        domain: 'folksonomia-digital/user-contribution-node/v1',
        tag: tag.tag_original,
        eixo,
        id: tag.id,
      });
    ...
    for (const mem of (memoriaDB || [])) {
      ...
      memoriaNodes.push({
        ...
        hash: generateSignature({
          domain: 'folksonomia-digital/semantic-memory-node/v1',
          termo: mem.termo,
        }),
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (action === 'ingest' && tag) {
      ...
      const hash = generateSignature({
        domain: 'folksonomia-digital/semantic-graph-ingest/v1',
        tag,
        eixo,
