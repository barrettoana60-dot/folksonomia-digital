...
function generateSemanticDna(payload: any): string {
  return generateCrossReferenceHash(payload);
}
  ...
  try {
    ...
    try {
      await supabaseAdmin.from('eventos').insert({
        ...
        hash_evento: hashDna,
        payload: {
          payload_hash: generateSignature(dnaPayload),
          cross_hash: hashDna,
          encrypted_payload: encryptPayload(dnaPayload),
    ...
    if (error instanceof EncryptionConfigurationError) {
      return NextResponse.json({
        error: 'Cofre bloqueado: configure ENCRYPTION_KEY antes de criar uma ligação auditável.',
