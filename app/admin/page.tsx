...
export default function AdminPage() {
  ...
  const [analysisParams, setAnalysisParams] = useState({
    profundidade: 'PADRAO' as 'RAPIDA' | 'PADRAO' | 'PROFUNDA',
  ...
  const [interopNodes, setInteropNodes] = useState<any[]>([]);
  /* Catálogo estático legado removido: o Cofre Semântico usa somente registros de usuários.
    // ═══ NÓ CENTRAL ═══
  ...
  const [interopConnections, setInteropConnections] = useState<any[]>([]);
  /* Conexões estáticas legadas removidas junto com o catálogo fixo.
  ...
  // Carregar rede cultural persistida + métricas reais de DL
  const fetchInteropNetwork = useCallback(async () => {
    try {
      const res = await fetch('/api/admin/interop-network');
      const json = await res.json();
      if (json.success && json.data) {
        const { nodes, edges, metrics } = json.data;
        if (nodes?.length > 0) {
          setInteropNodes(current => {
            const existingIds = new Set(current.map(n => n.id));
            const merged = [...current];
            for (const n of nodes) {
              if (!existingIds.has(n.id)) {
                merged.push({ ...n, vx: 0, vy: 0 } as any);
              }
            }
            return merged;
          });
        ...
        if (edges?.length > 0) {
          setInteropConnections(current => {
            const existingKeys = new Set(current.map(c => `${c.from}-${c.to}`));
            const merged = [...current];
            for (const e of edges) {
              const key = `${e.from}-${e.to}`;
              const revKey = `${e.to}-${e.from}`;
              if (!existingKeys.has(key) && !existingKeys.has(revKey)) {
                merged.push({ ...e, isNew: e.discovered } as any);
              }
            }
            return merged;
          });
  ...
  const fetchLearningMetrics = useCallback(async () => {
    try {
      const res = await fetch('/api/ml/learning-status');
      const json = await res.json();
      if (json.success && json.data) {
        setRealMetrics(json.data);
        if (json.data.nnLoss !== undefined) {
          nnLossRef.current = json.data.nnLoss;
          setNnLoss(json.data.nnLoss);
        }
        if (json.data.nnAccuracy !== undefined) {
          nnAccuracyRef.current = json.data.nnAccuracy;
          setNnAccuracy(json.data.nnAccuracy);
        }
        if (json.data.epoch !== undefined) {
          nnEpochRef.current = json.data.epoch;
          setNnEpoch(json.data.epoch);
        }
      }
    } catch { /* silent */ }
  }, []);
  ...
  const handleSemanticSearch = async (overrideTag?: string) => {
    ...
    try {
      ...
      if (json.success) {
        ...
        const fontesAcademicas = json.data.fontesAcademicas || [];
        ...
        setInteropNodes(currentNodes => {
          ...
          let updated = exists
            ? currentNodes.map(n => n.id.toLowerCase() === tagKey ? { ...n, activation: 1.0 } : n)
            : [...currentNodes, {
                id: tagKey,
                label: tag,
                x: 400 + Math.cos(Math.random() * Math.PI * 2) * (180 + Math.random() * 50),
                y: 215 + Math.sin(Math.random() * Math.PI * 2) * (180 + Math.random() * 50),
                size: 14,
                fill: "#6D28D9",
                eixo: "PATRIMONIO",
                ...
                vx: 0, vy: 0, activation: 1.0,
                linksReais: fontesAcademicas.slice(0, 3).map((a: any) => ({
                  label: a.titulo?.substring(0, 40) || 'Artigo Acadêmico',
                  url: a.link || 'https://brasiliana.museus.gov.br',
                })),
              }];

          // 1b. Injetar nós de artigos acadêmicos no grafo de interoperabilidade
          fontesAcademicas.slice(0, 4).forEach((art: any, idx: number) => {
            const artId = `artigo_${tagKey}_${idx}`;
            if (!updated.some(n => n.id === artId)) {
              const angle = (idx / Math.max(fontesAcademicas.length, 1)) * Math.PI * 2;
              updated = [...updated, {
                id: artId,
                label: (art.titulo || 'Artigo').substring(0, 30),
                x: 400 + Math.cos(angle) * 120,
                y: 215 + Math.sin(angle) * 120,
                size: 10,
                fill: "#E8A920",
                eixo: "PATRIMONIO",
                desc: art.descricao || `Publicação acadêmica: ${art.autores || 'Autor desconhecido'}`,
                type: "Artigo Científico (RAG)",
                hash: `art_dna_${Math.random().toString(16).slice(2, 6)}`,
                vx: 0, vy: 0, activation: 0.8,
                linksReais: [{ label: art.titulo || 'Ver publicação', url: art.link || '#' }],
                acervos: [art.fonte || 'OpenAlex'],
              }];
            }
          });
          return updated;
        ...
        // 2. Conectar a tag ao núcleo central e aos artigos acadêmicos
        setInteropConnections(currentConns => {
          let conns = [...currentConns];
          const coreExists = conns.some(c =>
            (c.from === 'core' && c.to === tagKey) || (c.from === tagKey && c.to === 'core')
          );
          if (!coreExists) {
            conns.push({ from: "core", to: tagKey, weight: 0.82 + Math.random() * 0.12, discovered: true });
          }
          // Conectar tag aos artigos acadêmicos encontrados
          fontesAcademicas.slice(0, 4).forEach((art: any, idx: number) => {
            const artId = `artigo_${tagKey}_${idx}`;
            const artConnExists = conns.some(c =>
              (c.from === tagKey && c.to === artId) || (c.from === artId && c.to === tagKey)
            );
            if (!artConnExists) {
              conns.push({
                from: tagKey,
                to: artId,
                weight: 0.65 + Math.random() * 0.2,
                discovered: true,
                eixoRel: "PATRIMONIO",
              });
            }
            // Conectar artigo ao nó artigo_popular existente
            const popConnExists = conns.some(c =>
              (c.from === 'artigo_popular' && c.to === artId) || (c.from === artId && c.to === 'artigo_popular')
            );
            if (!popConnExists) {
              conns.push({
                from: 'artigo_popular',
                to: artId,
                weight: 0.55 + Math.random() * 0.15,
                discovered: true,
                eixoRel: "PATRIMONIO",
              });
            }
          ...
          setNnDiscovered(prevDisc => [
            {
              ...
              from: "core",
              to: tagKey,
              label: `RAG: "${tag}" integrado à malha de DNA Semântico${fontesAcademicas.length > 0 ? ` (+${fontesAcademicas.length} artigos)` : ''}`,
              epoch: nnEpochRef.current,
              confidence: json.data.motores?.transformer?.certeza || 96
            },
            ...prevDisc
          ].slice(0, 12));
        }

        // Log de deep learning + cadeias culturais
        if (fontesAcademicas.length > 0) {
          setDlLog(prev => [{
            tag,
            resultado: `${fontesAcademicas.length} fonte(s) acadêmica(s) indexada(s) — ${fontesAcademicas.map((a: any) => a.fonte).join(', ')}`,
            ts: new Date().toLocaleTimeString('pt-BR'),
          }, ...prev].slice(0, 20));
        }

        // Cadeias descobertas na rede cultural
        if (json.data.redeCultural?.chains?.length > 0) {
          setChainLog(prev => [
            ...json.data.redeCultural.chains.map((insight: string) => ({
              insight,
              ts: new Date().toLocaleTimeString('pt-BR'),
            })),
            ...prev,
  ...
  // A tela do Cofre Semântico carrega somente contribuições persistidas pelo
  // próprio componente. O antigo simulador não inicia nem injeta nós fixos.
  useEffect(() => {
    stopTraining();
  }, [activeTab, stopTraining]);
          ...
          <>
              ...
              <div className="space-y-8 animate-fade-in">
                 ...
                 {showAddForm && (
                   <div className="fixed inset-0 z-50 bg-black/20 backdrop-blur-md flex items-start justify-center overflow-y-auto p-4 md:p-8">
                     <div className="obra-modal-shell p-6 md:p-8 w-full max-w-2xl max-h-[calc(100vh-2rem)] md:max-h-[calc(100vh-4rem)] overflow-y-auto relative animate-fade-in my-auto">
            ...
            {activeTab === 'relatorios' && (
              <div className="space-y-8 animate-fade-in print-section">
                ...
                <div className="glass-card p-8 md:p-10 space-y-6 print:hidden">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                      ...
                      <p className="text-[10px] uppercase tracking-wider text-[#1A1A1A]/40 mt-0.5">Contagem diária de tags inseridas na rede de contribuições dos usuários</p>
                    </div>
                ...
                <div className="glass-card p-6 space-y-4 print:hidden !bg-white/62 backdrop-blur-xl border border-white/50 shadow-xl">
                  ...
                  <div className="border-t border-black/5 pt-4">
                    <button
                      ...
                      className="text-[10px] uppercase font-semibold tracking-wider text-[#1A1A1A]/50 hover:text-[#E85002] flex items-center gap-1.5 transition-colors"
                    >
                      <Settings size={12} /> Parâmetros de Análise {showAdvancedParams ? '▲' : '▼'}
                    </button>
                    {showAdvancedParams && (
                      <div className="mt-3 grid grid-cols-2 md:grid-cols-4 gap-3">
                        <div>
                          <label className="text-[9px] uppercase font-semibold tracking-wider text-[#1A1A1A]/40 block mb-1">Profundidade</label>
                          <select
                            value={analysisParams.profundidade}
                            onChange={e => setAnalysisParams(p => ({ ...p, profundidade: e.target.value as any }))}
                        ...
                        <div>
                          <label className="text-[9px] uppercase font-semibold tracking-wider text-[#1A1A1A]/40 block mb-1">Max. Artigos</label>
                          <input type="number" min={2} max={15} value={analysisParams.maxArtigos}
                        ...
                        <label className="flex items-center gap-2 text-xs text-[#1A1A1A]/70 cursor-pointer">
                          <input type="checkbox" checked={analysisParams.incluirAcademico}
                            ...
                            className="accent-[#E85002]" /> Fontes Acadêmicas
                        </label>
                        <label className="flex items-center gap-2 text-xs text-[#1A1A1A]/70 cursor-pointer">
                          <input type="checkbox" checked={analysisParams.incluirAcervos}
                ...
                {semanticResult && (
                  <div className="space-y-6">
                    ...
                    {/* FONTES ACADÊMICAS — Artigos com links clicáveis */}
                    {semanticResult.fontesAcademicas?.length > 0 && (
                      <div className="glass-card p-6 border border-[#E8A920]/30 space-y-4">
                        <h4 className="text-xs font-semibold uppercase tracking-wider flex items-center gap-2">
                          <BookOpen size={16} className="text-[#E8A920]" />
                          Fontes Acadêmicas ({semanticResult.fontesAcademicas.length})
                        </h4>
                        <div className="space-y-3">
                          {semanticResult.fontesAcademicas.map((art: any) => (
                            <div key={art.id} className="p-4 bg-[#E8A920]/5 border border-[#E8A920]/15 rounded-lg hover:border-[#E8A920]/30 transition-colors">
                              <div className="flex items-start justify-between gap-3">
                                <div className="flex-1 min-w-0">
                                  <p className="text-sm font-semibold text-[#1A1A1A] leading-snug">{art.titulo}</p>
                                  {art.autores && (
                                    <p className="text-[11px] text-[#1A1A1A]/60 mt-1">
                                      {art.autores}{art.ano ? ` · ${art.ano}` : ''}{art.revista ? ` · ${art.revista}` : ''}
                                    </p>
                                  )}
                                  {art.descricao && (
                                    <p className="text-[11px] text-[#1A1A1A]/50 mt-2 line-clamp-2">{art.descricao.replace(/\*\*/g, '')}</p>
                                  )}
                                  <div className="flex flex-wrap gap-2 mt-2">
                                    <span className="px-2 py-0.5 bg-[#E8A920]/10 text-[#E8A920] rounded text-[9px] font-bold uppercase">{art.fonte}</span>
                                    {art.tipo && <span className="px-2 py-0.5 bg-black/5 text-[#1A1A1A]/50 rounded text-[9px] uppercase">{art.tipo}</span>}
                                  </div>
                                </div>
                                <div className="flex flex-col gap-1.5 shrink-0">
                                  {art.link && (
                                    <a href={art.link} target="_blank" rel="noopener noreferrer"
                                      className="flex items-center gap-1 px-3 py-1.5 bg-[#E85002] text-white rounded text-[10px] font-semibold hover:bg-[#c73d08] transition-colors">
                                      <ExternalLink size={11} /> Acessar
                                    </a>
                                  )}
                                  {art.doi && (
                                    <a href={`https://doi.org/${art.doi}`} target="_blank" rel="noopener noreferrer"
                                      className="flex items-center gap-1 px-3 py-1.5 bg-white/60 border border-black/10 rounded text-[10px] font-semibold text-[#1A1A1A]/70 hover:bg-white transition-colors">
                                      DOI
                                    </a>
                                  )}
                                </div>
                              ...
                              {art.citacaoAbnt && (
                                <p className="text-[10px] text-[#1A1A1A]/40 mt-3 pt-2 border-t border-black/5 italic">
                                  {art.citacaoAbnt.replace(/\*\*/g, '')}
                                </p>
                              )}
                            </div>
                          ))}
                    ...
                    {semanticResult.relatorioEstruturado ? (
                      <div className="grid grid-cols-1 gap-4 mt-6">
                        {semanticResult.relatorioEstruturado.camadas ? (
                          <>
                            {/* Seção de Literatura Acadêmica do relatório */}
                            {semanticResult.relatorioEstruturado.camadas.evidenciaEmpirica && (
                              <div className="glass-card p-8 border-l-4 border-[#E8A920]/50">
                                <div className="text-[#1A1A1A] space-y-3 font-normal">
                                  {renderMarkdown(semanticResult.relatorioEstruturado.camadas.evidenciaEmpirica)}
                                </div>
                              </div>
                            )}
                            {/* Objetos do Acervo */}
                            {semanticResult.relatorioEstruturado.camadas.extracao && (
                              <div className="glass-card p-8 border-l-4 border-blue-500/50">
                                <div className="text-[#1A1A1A] space-y-3 font-normal">
                                  {renderMarkdown(semanticResult.relatorioEstruturado.camadas.extracao)}
                                </div>
                              </div>
                            )}
                   ...
                   <div className="fixed inset-0 z-50 bg-white/80 flex items-center justify-center p-4" onClick={() => setGraphNodeSelected(null)}>
                     <div className="glass-card p-8 w-full max-w-2xl relative animate-fade-in max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                       ...
                       <div className="flex gap-3 mt-4">
                         <button onClick={() => { setSearchTag(graphNodeSelected); setActiveTab('tags'); setGraphNodeSelected(null); setTimeout(() => handleSemanticSearch(), 300); }} className="flex-1 liquid-button !bg-[#E85002] text-xs font-semibold tracking-wider">
                           Análise Semântica Completa
                         </button>
                         <button onClick={() => { setActiveTab('interoperabilidade'); setGraphNodeSelected(null); }} className="flex-1 liquid-button !bg-white/10 text-xs font-semibold tracking-wider">
                           Selar no Cofre Cultural
                         </button>
                       </div>
                     </div>
                   </div>
