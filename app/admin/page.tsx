  ...
  return (
    <main className="min-h-screen pt-24 pb-20 px-8">
      <div className="max-w-[1400px] mx-auto space-y-12">
        ...
        {/* Header Tabs */}
        <nav className="flex flex-wrap items-center gap-2 border-b border-white/5 pb-6">
          {tabs.map(tab => (
            <button
              key={tab.id}
        ...
        {activeTab === 'visao' && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 animate-fade-in">
            {stats.map((s, i) => (
              <div key={i} className="glass-card p-12 flex flex-col items-center text-center space-y-8">
                <div className="w-14 h-14 rounded-full border border-white/10 flex items-center justify-center bg-white/5">
                  <s.icon size={28} style={{ color: s.color }} />
                </div>
                <div className="space-y-2">
                  <p className="text-5xl font-normal serif-title text-white tracking-tight">{s.value}</p>
                  <p className="text-[10px] uppercase tracking-[0.25em] text-white/30 font-black">{s.label}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {activeTab === 'obras' && (
          <div className="space-y-8 animate-fade-in">
            <div className="flex justify-between items-center">
              <h2 className="text-3xl font-normal serif-title uppercase tracking-widest">Acervo Institucional</h2>
              <button 
                onClick={() => setShowAddForm(!showAddForm)}
                className="liquid-button !bg-[#E85002] !text-white !border-none !rounded-full flex items-center gap-3"
              >
            ...
            {showAddForm && (
              <div className="glass-card p-10 border-[#E85002]/30 space-y-8 animate-fade-in">
                <h3 className="text-sm font-bold uppercase tracking-widest">Novo Registro de Obra</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <textarea placeholder="Descrição Institucional" className="liquid-input w-full md:col-span-2 h-32" />
                </div>
            ...
            <div className="space-y-6">
              {[
                ...
                <div key={obra.id} className="glass-card p-8 flex items-center justify-between group hover:border-white/20 transition-all">
                  <div className="flex items-center gap-10">
                    ...
                    <div>
                      <h3 className="text-xl font-bold tracking-tight">#{obra.id} — {obra.titulo}</h3>
                      <p className="text-white/40 text-xs font-bold uppercase tracking-[0.2em] mt-2">{obra.artista} — {obra.ano}</p>
                    </div>
                  </div>
          ...
          <div className="space-y-8 animate-fade-in">
            ...
            <div className="glass-card overflow-hidden">
              <table className="w-full text-left text-sm">
                <thead className="bg-white/5 border-b border-white/10 text-[10px] uppercase tracking-widest font-black text-white/50">
                  <tr>
                    <th className="px-10 py-6">Visitante</th>
                    <th className="px-10 py-6">Tag Original</th>
                    <th className="px-10 py-6">Ressonância</th>
                    <th className="px-10 py-6">Status Semântico</th>
                ...
                <tbody className="divide-y divide-white/5">
                  {[
                    ...
                    { user: 'Visitante #C1', tag: 'Paz', score: '82%', status: 'Validado' },
                  ].map((row, i) => (
                    <tr key={i} className="hover:bg-white/[0.02] transition-colors">
                      <td className="px-10 py-8 font-bold text-white/60">{row.user}</td>
                      <td className="px-10 py-8 text-lg serif-title">{row.tag}</td>
                      <td className="px-10 py-8"><span className="text-[#E85002] font-mono font-bold">{row.score}</span></td>
                      <td className="px-10 py-8"><span className="px-4 py-1.5 rounded-full bg-white/5 text-[9px] font-black uppercase border border-white/10">{row.status}</span></td>
                      <td className="px-10 py-8">
                        <button className="liquid-button !py-2 !px-4 !text-[9px] !rounded-lg">Validar</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
