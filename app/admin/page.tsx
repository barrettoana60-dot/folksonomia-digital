          ...
          <>
            ...
            {activeTab === 'interoperabilidade' && (
              <CulturalInteroperabilityView
                initialNodes={interopNodes}
                initialConnections={interopConnections}
                onTriggerRAG={async (nodeLabel: string) => {
                  try {
                    const res = await fetch('/api/admin/relatorio-semantico', {
                      method: 'POST',
                      headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify({ tag: nodeLabel })
                    });
                    const json = await res.json();
                    if (json.success) {
                      fetchInteropNetwork();
                    ...
                    console.error('[RAG] Erro ao treinar nó:', e);
                  }
                }}
