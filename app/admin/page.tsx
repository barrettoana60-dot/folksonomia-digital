...
export default function AdminPage() {
  ...
  }, [activeTab, startTraining, stopTraining, fetchInteropNetwork, fetchLearningMetrics]);

  // Polling de métricas reais de DL a cada 15s na aba interoperabilidade
  useEffect(() => {
    if (activeTab !== 'interoperabilidade') return;
    const interval = setInterval(fetchLearningMetrics, 15000);
    return () => clearInterval(interval);
  }, [activeTab, fetchLearningMetrics]);
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-black/10 pb-5">
                  <div>
                    ...
                    <p className="text-xs text-[#1A1A1A]/50 mt-1 uppercase tracking-widest font-semibold">
                      Rede cadeada complexa — Hebbian + propagação A→B→C + aprendizado progressivo contínuo
                    </p>
                  </div>
                    ...
                    <div className="glass-card p-5 border border-black/07 space-y-4">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {realMetrics && (
                            <span className="text-[8px] font-mono text-green-600 uppercase tracking-widest">
                          ...
                          <button
                            onClick={async () => {
                              try {
                                const res = await fetch('/api/ml/learning-status', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ limit: 2 }) });
                                const json = await res.json();
                                if (json.success) fetchLearningMetrics();
                              } catch { /* silent */ }
                            }}
                            className="text-[8px] uppercase font-bold px-2 py-1 bg-[#6D28D9]/10 text-[#6D28D9] rounded hover:bg-[#6D28D9]/20 transition-colors"
                          >
                            Treinar
                          </button>
                          <button
                      ...
                      {/* Cadeias culturais descobertas (propagação A→B→C) */}
                      {chainLog.length > 0 && (
                        <div className="space-y-1.5 max-h-36 overflow-y-auto">
                          <p className="text-[8px] uppercase font-bold tracking-wider text-[#E8A920]">Cadeias Culturais ({chainLog.length})</p>
                          {chainLog.map((c, i) => (
                            <div key={i} className="p-2 bg-[#E8A920]/5 border border-[#E8A920]/15 rounded-lg text-[8px] text-[#1A1A1A]/70">
                              <span className="text-[#E8A920] font-bold mr-1">⛓</span>{c.insight}
                              <span className="text-[#1A1A1A]/30 ml-2">{c.ts}</span>
                            </div>
                          ))}
                        </div>
                      )}
                      ...
                      {nnDiscovered.length === 0 && dlLog.length === 0 && chainLog.length === 0 ? (
